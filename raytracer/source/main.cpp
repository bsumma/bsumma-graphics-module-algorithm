...
vec4 castRay(vec4 p0, vec4 dir, Object *lastHitObject, int depth){
  ...
  if(depth > maxDepth){ return vec4(0.0,0.0,0.0,0.0); }
  
  for(unsigned int i=0; i < sceneObjects.size(); i++){
    Object::IntersectionValues isect = sceneObjects[i]->intersect(p0, dir);
    if(isect.t_w != std::numeric_limits< double >::infinity()){
...
void rayTrace(){
  ...
  std::cout << GLState::window_width << GLState::window_height << "\n";
  std::string file = source_path + "/output.png";
  write_image(file.c_str(), buffer, GLState::window_width, GLState::window_height, 4);
